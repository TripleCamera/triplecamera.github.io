...
<img src="archlinux-power.png" alt="电源管理方案不可用的提示" style="max-height: 20em;">
...
<img src="archlinux-vlc-nohidpi.png" alt="没有启用 HiDPI 的 VLC 界面" style="max-height: 13em;">
...
<img src="archlinux-vlc-hidpi.png" alt="启用了 HiDPI 的 VLC 界面" style="max-height: 23em;">
...
![修改前的分区](archlinux-partition-before.jpg)
...
![修改后的分区](archlinux-partition-after.jpg)
...
![IntelliJ IDEA 截图，可以看到微软雅黑字形高度不一，非常难看](archlinux-yahei.png)
...
首先，AUR 里只有[<samp>llvm12</samp>](https://aur.archlinux.org/packages/llvm12)<sup>AUR</sup>，没有 <samp>clang12</samp>。其次，这个 clang12 编译还会报错。
...
直接搜索报错信息，找到了一个 [GitHub issue](https://github.com/JuliaLang/julia/issues/49581)，而且被很多 issue 引用过。issue 里面还有一个 [commit](https://github.com/JuliaLang/llvm-project/commit/ff1681ddb303223973653f7f5f3f3435b48a1983) 链接。

传统的方法（克隆、构建、安装）。但是为了省事，我的解决方案是：启动安装，拼手速改代码。哈哈，最后顺利编译成功了（花了大约 40min）。

注：我在评论区留下了修复的方法，但作者一直没有回我。于是我就开始考虑把 <samp>llvm12</samp> 接手下来。我计划寒假再搞。
...
![系统设置中“声音”一栏的截图。上面显示，我的声卡已停用，需要手动将其开启](archlinux-soundcard.png)
