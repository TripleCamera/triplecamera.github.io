...
## 缘起
为什么我会想到考证 MOS 的历史呢？这还要从我重做课下任务说起。我去年做实验的时候，被代码和指导书的各种问题折磨，吃了不少苦；今年做助教的时候，自然希望能改掉实验课的诸多问题——但当时想要改掉的问题已经被我忘光光了。于是我认为，要想发现实验课的不足，就要设身处地，重做一遍课下任务。可惜的是，许多助教并不看重这一点，他们认为，既然去年吃了一遍苦，今年又何必再吃呢？于是他们就没有重做课下，自然也就少了很多发现。
...
我不禁感到疑惑：**MOS 的代码质量为何差强人意**？MIT JOS 的代码质量本来也是这样的吗？还是说在移植的过程中出现了质量下降的问题？
...
在这些回答中，许多被害惨了的同学纷纷吐槽这门课程的不足。其中不乏一些**尖锐的批评**，让我们看到了这门课程在几年前是多么的不友好。另外还有同学拿这门课与 6.828 作比较，希望课程能多多吸收 6.828 的优点。

值得庆幸的是，最后一个回答发布于 2022 年——这说明近几年我们这门课并没有什么逆天操作，让同学把课程组挂到知乎上（笑）。

### 旧版指导书
OS 实验有一本厚厚的[指导书](https://os.buaa.edu.cn/public/guide-book.pdf)。每年都会发布新的指导书，课程组也提醒我们不要用旧版指导书。但在去年做实验的过程中，我就对这门课程的历史产生了好奇，于是四处搜寻往届指导书。我从往届的 GitHub 仓库里翻到了学长顺手传上去的指导书，有 2023 年、2021 年、2019 年的版本。但紧接着，我找到了宝藏——[旧版指导书的源代码](https://github.com/SivilTaram/BUAAOS-guide-book)！

根据 README 的介绍，这门课程早期的助教们于 2015 年 7 月决定要编写指导书，并在 2016 年 3 月开始编写。从开始编写以来，指导书一直在 GitHub 上开源，直到 2018 年改为自建 GitLab 托管。Releases 中有编译出来的 PDF，最后一版 v2.1.4 发布于 2017 年 10 月 10 日。

这版指导书与我们现在使用的指导书有很大不同：书名叫《小操作系统实验指导书》；没有“引言”部分，取而代之的是“编者序”~~（非常中二）~~“教师寄语”，以及一张大事表，上面记载着操作系统实验课程自 1999 年以来的发展。

<figure>
  <img src="mos-guide-book.png" alt="旧版指导书中的“MIPS 操作系统实验大事表”" style="max-height: 12em">
...
<figure>
  <img src="mos-course-team.png" alt="操作系统实验课程宣发页（截取于 2025 年 5 月 1 日）" style="max-height: 18em">
  <figcaption>操作系统实验课程宣发页（截取于 2025 年 5 月 1 日）</figcaption>
</figure>

值得一提的是，指导书在 2023 年还正式出版过，名字叫《[MOS 操作系统实验教程](https://xuanshu.hep.com.cn/front/book/findBookDetails?bookId=64417020938b7cc2960efce5)》，是一本狼书。感兴趣的同学可以去图书馆借一本看看。

### 旧版代码
对于操作系统实验这门课来说，GitHub 上最有价值的并不是指导书，而是学长们的代码！大多数同学在完成实验的时候，或多或少都参考过学长的代码。值得一提的是，考虑到大家课下大量借鉴学长的代码，课下题目约等于送分，加上部分学长代码有 bug，会在课上考试时爆雷；因此课程组在今年开源了 MOS 操作系统代码（含答案）（[GitHub](https://github.com/buaa-os/mos.public) &bull; [Gitee](https://gitee.com/osbuaa/mos)），大家终于可以参考 100% 正确的官方代码了！

大家在参考学长代码的时候，肯定是越新越好。因为 MOS 每年都有一些小改动，太旧的代码可能没有办法正常工作。但我的目的是考古，所以代码肯定是越旧越好。就这样，在花费了一些时间翻找后，我居然找到了一份 [2015 年的 MOS 源码](https://github.com/SivilTaram/Jos-mips)！源码仓库的作者也是旧版指导书仓库的作者，他叫 [SivilTaram](https://github.com/SivilTaram)，根据仓库中遗留的线索推断，应该就是大事表中所说的刘乾学长。下面让我们来好好研究一下这个仓库：
...
<figure>
  <img src="mos-jos-mips.png" alt="使用 github.dev 预览旧版代码" style="max-height: 18em">
  <figcaption>使用 github.dev 预览旧版代码</figcaption>
</figure>
...
由于我有幸担任了 2025 年操作系统实验助教，因此可以接触到内部资料：指导书和 MOS 两个仓库的提交记录。这些内部资料为考证工作带来了方便，但没有它们也能得出下文的结论。
 ...
 -  指导书在 v2.1.4（2017 年 10 月 10 日）及以前的提交记录与 GitHub 上的提交记录完全重合。
 -  MOS 提交记录始于 2022 年 9 月 4 日。尚不清楚在此之前是使用何种方式分发代码的。
     -  根据我在当助教期间得到的信息，MOS 最开始使用的模拟器是 [GXemul](https://en.wikipedia.org/wiki/GXemul)，CPU 架构为 [MIPS R3000](https://en.wikipedia.org/wiki/R3000)。近两年才改为使用 [QEMU](https://en.wikipedia.org/wiki/QEMU) 和 MIPS32 4Kc。

## MIT 操作系统课程
接下来，让我介绍一下 MIT 的操作系统课程。首先，建议阅读一下 [CS 自学指南（csdiy.wiki）](https://csdiy.wiki/%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F/MIT6.S081/) 对 MIT 6.S081（原名 6.828）这门课程的介绍。
...
最开始 MIT 并没有操作系统课。2002 年，MIT 开设了 [6.097: OPERATING SYSTEM ENGINEERING](https://pdos.csail.mit.edu/archive/6.097/) 课程。2003 年，课程代码更改为 [6.828](https://pdos.csail.mit.edu/6.828/2003/)。这门课程共分为三个部分（[参考链接](https://pdos.csail.mit.edu/6.828/2003/overview.html)）：
 -  **讲座**：课程前半部分讲解 [UNIX v6](https://en.wikipedia.org/wiki/Version_6_Unix) 操作系统的源代码，后半部分讲解操作系统的重要概念。
 -  **阅读**：课程前半部分的阅读材料为参考书《[Lions' Commentary on UNIX 6th Edition](https://en.wikipedia.org/wiki/A_Commentary_on_the_UNIX_Operating_System)》（中文版：《莱昂氏UNIX源代码分析》），后半部分的阅读材料为各种论文。
 -  **实验**：2003 年共有 7 个实验（Lab），从 Lab0 到 Lab6，学生需要从头构建一个操作系统。
     -  学生需要从官网下载 `.tar.gz` 格式的实验代码。切换到下一个 Lab 时，需要手动应用上一个 Lab 所做的更改。

在接下来的十几年中，这门课程的结构并没有发生太大变化，让我列举其中的一些重要事件：
 -  2004 年，学生构建的操作系统被命名为 JOS。（[参考链接](https://pdos.csail.mit.edu/6.828/2004/overview.html)）在本文中，为了方便称呼，我将 2004 年以前的版本也称作 JOS。
 -  2006 年，由于 UNIX v6 过于老旧，课程团队将其移植到了现代 C 语言和现代 x86 架构上。新系统被称为 [xv6](https://en.wikipedia.org/wiki/Xv6)，从此课上改为讲解 xv6 源代码。（[参考链接](https://pdos.csail.mit.edu/6.828/2006/overview.html)）
 -  2008 年，课程组改用 Git 分发实验代码。切换到下一个 Lab 时，可以直接使用 `git merge` 命令进行分支合并。

到了 2019 年，这门课程发生了翻天覆地的变化：课程代码更改为 6.S081，由研究生课程变为本科生课程。同时难度大幅降低，从完成 JOS 操作系统（x86 架构）变为改进 xv6 操作系统（RISC-V 架构）。最后一次开课是在 2024 年秋季。

更多细节可以在上文提到的 `mit-6.828-lab-files` 项目中的 README 中找到。

## MOS 断代工程
我们已经准备好了 MIT 6.828 的历届代码，以及一份 2015 年的 Jos-mips 代码。接下来，我们就可以开始为 MOS 做一次“断代”了。

根据指导书中的大事表，课程组在 2007 年发现并完成了 JOS 实验，同时开始了移植工作，到 2009 年完成移植。那么移植的到底是 2007 年还是 2009 年的版本呢？我决定把 MIT 每一年的代码都拿出来比较。
...
这些工作花费了我两天的时间——4 月 15 日和 17 日。

### MIT OpenCourseWare
就在断代工作被迫中止的同一天，事情又迎来了新的转机。就在 17 日当天，不甘心的我试图在网上搜索 [MIT 6.828 2003](https://www.google.com/search?q=MIT+6.828+2003)，结果意外发现了 MIT DSpace（MIT 数字图书馆）上的 OpenCourseWare Archive，里面有 2003 年的存档。
...
<figure>
  <img src="mos-ocw2003-kompare.png" alt="使用 Kompare 对比两个版本的 lab1.zip 的十六进制文本文件" style="max-height: 18em">
...
### 为什么 `KADDR` 不会对 `pa` 做类型转换？
我们可以在 `inc/mmu.h` 中找到 `KADDR` 的定义：
...
### 更多发现
MOS 的 `user/include/fs.h` 中记录了文件系统魔数（magic number）：

```c
#define FS_MAGIC 0x68286097 // Everyone's favorite OS class
```

这是同学们喜闻乐道的小彩蛋，体现了我们这门课程来自 MIT 6.828。往年甚至有同学傻乎乎地去[讨论区](https://triplecamera.github.io/os-discussions/208)问这串数字是什么意思，大家只是点赞不说话。

唉，让我给这些云里雾里的同学解释一下吧！

既然有了 JOS 的源代码，我们就可以很轻易地在 JOS 的源代码中找到这一句（`inc/fs.h`）：

```c
#define FS_MAGIC	0x68286097	// Everyone's favorite OS class
```

前四位 6828 显然代表 6.828，那后四位 6097 呢？如果你认真读了这篇文章的前半部分，你会认识到它们代表 6.097，也就是这门课程在前一年的旧代码。2003 年引用 2002 年的代码，非常合理吧。

另外 2006 年，这个魔数变成了 `0x4A0530AE`，旁边的注释是“related vaguely to 'J\0S!'”（`4A` 是 J 的 ASCII 码，`53` 是 S 的 ASCII 码，中间的 `0` 和 O 相似，大概吧）。2006 年以后很长一段时间似乎都没有变，04、05 年什么样我就不知道了。

---

北航与 MIT 指导书在 Lab6 中管道竞争部分的内容相似度很高，说明当初编写指导书时肯定参考了 MIT 的指导书。可惜 MIT 的指导书没能流传下来，也许是不小心弄丢了。（当然，也不排除另一种可能：学长们编写完指导书后觉得 MIT 的指导书没用了，就故意扔掉了。）

<figure>
  <img src="mos-lab6-buaa.png" alt="北航指导书 Lab6 中管道竞争部分的内容" style="max-height: 12em">
  <figcaption>北航指导书 Lab6 中管道竞争部分的内容</figcaption>
</figure>
<figure>
  <img src="mos-lab6-mit.png" alt="MIT 指导书 Lab6 中管道竞争部分的内容" style="max-height: 18em">
  <figcaption>MIT 指导书 Lab6 中管道竞争部分的内容</figcaption>
</figure>
...
首先，通过考证，加深了我们对 MOS 历史悠久这一点的认识。在此之前，我们一直以为 MOS 的历史从 2007 年开始；但这些工作将 MOS 的历史上溯到了 2003 年。许多同学都认为 MOS 已经过时了。诚然如此。从 2003 年到 2025 年，MIT 的操作系统课程一直在迭代，到现在已经发生了翻天覆地的变化。但 MOS 从 2009 年移植到 MIPS 架构以来，整体的课程设计并没有发生大的变化。虽然助教们做了许多移植方面的工作，比如将 MOS 从 GXemul 移植到了 QEMU，从 R3000 移植到 4Kc，但这些变化始终没有触及到根本。
...
我的助教生涯已经结束了。希望后面的助教可以学习一下 MIT 6.828 2003，然后对 MOS 进行改进。

## 感想
写到这里，回顾我这一学期以来的考证工作，不由得感慨万千。我打算把我的这些感想都留在这里。

操作系统课每年都要培养五百多名学生。这五百多人中，有多少人曾经对 MOS 的历史产生过好奇呢？操作系统课每年都要招收约二十名助教。这二十人中，有多少人对 MOS 的代码质量产生过疑惑呢？

这使我想起另一件事来了。有学生在讨论区里询问 Thinking 4.9 的第一问是否有问题。我和另几位助教讨论以后，一致认为有问题，但由于出题的助教没有留下答案，我们也无法得知他的意图，只好打算把这一问删掉。

经过考证，这道思考题是 2018 年加入的，距今已有七年时间。没想到这七年间，大部分同学都在想方设法地阐释这道思考题的合理性；可能有同学在心中产生过怀疑，也可能有同学向助教表达过质疑，但并没有获得助教的关注。这让我感到非常失望。指导书不是完美无缺的，MOS 也不是，学长学姐们留下的博客更不是。我们乐于看到大家的质疑，而非盲从。

二十年前的助教们学习 MIT JOS，并把它移植到 MIPS 平台上。十年前的助教们照着 MIT 的指导书编写北航自己的指导书，激昂文字，别管看不看得懂，反正激昂就对了。——那些历史已经离我们远去了。那些历史材料没有保存好，是学长们的失误，我们也无法挽回了。

着眼现在。我知道许多同学对 MOS 有所不满，对操作系统这门课程有所不满，乃至对计院、软院的培养方案也有所不满。这并没有什么问题，一方面，上一轮课程改革已是十年以前，经过十年的时间，你 6 系引以为傲的体系结构课已经僵化了。另一方面，考虑到这些年的内卷情况逐渐加重，助教们不得不更加关注自己的保研加分，课程改革缺乏动力。现在体系结构课程已经开始削减助教预算和人数了，未来甚至有取消保研加分的风险。改革已成为了遥遥无期的事情。
...
从去年 5 月开始撰写这篇文章以来，转眼已经过去半年了。其实本文的主体部分早在去年暑假就已经完成了，只剩下部分细节的补充，以及修改润色环节。但是因为考研和种种杂事一直拖到现在才完成。在这半年多的时间里，外部环境发生了一些变化：2406 取消了 STAR 助教的保研加分，MOS 在 2022 年以前的提交记录也找着了（其实它一直都在，只是我们都不知道而已）。因此，本文中描述的某些情况已经过时了——但是我不想再去写最近发生的这些事情了，这样下去真写不完了。因此，本文描述的情况会截止到去年暑假，也希望大家能体会到那段时间助教们对这些事情的看法。

最后感谢另两位 OS 助教帮我审阅这篇文章。就此匆匆定稿。

<p style="text-align: right">2026 年 2 月 17 日</p>
